...
CSP_2018_0 = CSP |> filter(year == 2018, main == 0)
CSP_2018_1 = CSP |> filter(year == 2018, main == 1) |> drop_na()
...
# 3. Prediction with Main -------------------------------------------------
  ...
  predic = predict(sfront, newdata = subset(CSP_2018, main == 0), asInData = TRUE )
  CSP_2018_0$Prediction = predic
  
  # Predict efficiency
  predic = 1-efficiencies(sfront, newdata = subset(CSP_2018, main == 0), asInData = TRUE)
  ...
  CSP_2018_0$Difference = exp(abs(distance_main_0))
  CSP_2018_0_table = CSP_2018_0 |> mutate(pred_ex = exp(Prediction), Difference = staff - pred_ex) |> 
    select(Difference, nome, year)
  xtable_obj = xtable(CSP_2018_0_table)
  print(xtable_obj, file = "CSP_2018_0_table.tex", include.rownames = FALSE, type = "latex")
  cat("Table saved as", "CSP_2018_0_table.tex", "\n")
  ...
  ggplot(data = CSP_2018, aes(x = log_app, y = log_staff, color = main)) +
    ...
    geom_point(aes(x = avg_app_main_1, y = avg_main_1), color = "yellow", size = 3, shape = 8)  +
