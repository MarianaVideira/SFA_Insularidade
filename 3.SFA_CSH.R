...
# 4. Azores Prediction: Continent Frontier --------------------------------
  ...
  CSH_effi = CSH_effi |> mutate(mean_efficiencyz = rowMeans(CSH_effi[,4:10], na.rm = TRUE),
                                mean_efficiencyz = rowMeans(CSH_effi[,11:16], na.rm = TRUE))
...
setwd("..")
  ...
  ### A. Select variables and test correlation ----------------------------
  CSH_panel = CSH |> select(hospital, year, operational, cmvmc, in_days, RO, azo, region, 
                        surg, urge, patient_dis, wait_scheduled_surg) |> 
    drop_na() |>
...
# 6. Operational Cost function: Azores Dummy ------------------------------
  ...
  ### A. Panel data -------------------------------------------------------
  CSH_panel = pdata.frame(CSH_0, c("hospital", "year"))
  ## 6.1 Define functional forms ------------------------------------------
  form = log(operational) ~ log(in_days) + log(surg) + log(urge) + log(RO) + azo
  formt = log(operational) ~ log(in_days) + log(surg) + I((log(in_days)^2)/2) + I((log(surg)^2)/2) + log(urge) + log(RO) + azo
  
  # Firm level inefficiencies models BC95
  # mean inefficiency of u_it is "determined" by factors z_it
  formz = log(operational) ~ log(in_days) + log(surg) + log(urge) + azo| RO + wait_scheduled_surg
  formtz = log(operational) ~ log(in_days) + log(surg) + log(urge) + I((log(in_days)^2)/2) + I((log(surg)^2)/2) + azo| RO + wait_scheduled_surg
  
  ## 6.2. Operational Cost function ----------------------------------------
  ### Estimation: time invariant inefficiencies ----------------------------
  ### A. Cobb-Douglas (form) -----------------------------------------------
  sfront = sfa( form,  
                data = CSH_panel,
                ineffDecrease = F, # FALSE for cost function and TRUE for production
...
# 7. Azores Prediction: Dummy Frontier -------------------------------------
  ## 6.1 Prediction --------------------------------------------------------
  CSH_predict = CSH_panel |> select(hospital, year, operational)
  ...
  CSH_effi = bind_rows(CSH_effi, efficiencies_sfa,efficiencies_sfat)
